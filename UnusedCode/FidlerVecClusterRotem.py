...
def FidlerVectorPartition(G):
    ...
    if np.sum(bool_vec)>1: #  if G is not connected, it is partition to its connected compoenets
        ...
        fidler_vec = eig_vec_sorted[:, 1].transpose()#fidler_vec = nx.linalg.algebraicconnectivity.fiedler_vector(G)
        l_dot_fidler = fidler_vec#fidler_vec*l
...
def Fidler_Clustering(G):
    ...
    while running_flag:
        ...
        lst = [g.number_of_nodes() for g in fidler_sub_G_list]
        print('Number:'+ str(np.sum(lst)) + ' Fidler_Work:' + str(lst))
...
"""
comparator: ClusteringComparator = ClusteringComparator(agents_num=node_qty, missions_num=5, cluster_size=sub_graph_node_qty)
G = nx.from_numpy_matrix(comparator.reduce_matrix(comparator.create_block_matrix()))
